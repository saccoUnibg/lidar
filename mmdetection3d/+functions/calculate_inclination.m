function [ground_equation_list, bike_equation_list, angle_list] = calculate_inclination(path, results_filtered, pcd_list)
    ...
    plot_pcd_ground(ground_equation_list,bike_equation_list,pcd_list,angle_list);
...
function bike_pcd = get_bike_pcd(pcd)
    intensity = double(pcd.Intensity());
    feat = zscore(intensity);                % standardizza
    [idx, C] = kmeans(feat, 2, 'Replicates',5, 'MaxIter',1000);
    
    % Heuristic: cluster con intensity media più alta -> probabile bici (metallo)
    muI = [mean(intensity(idx==1)), mean(intensity(idx==2))];
    [~,bikeLabel_byI] = max(muI);
    isBike = (idx == bikeLabel_byI);
    
    bike_pcd = select(pcd, ~isBike);
end

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
%  ---- Extra: plot piani + pcd ----
function plot_pcd_ground(ground_equation_list, bike_equation_list, pcd_list,angle_list)
    z_minimo = 999;
    z_massimo = -999;
    for i = 1 : size(pcd_list,1)
        ...
        if ~isempty(pcd)
            ...
            z_minimo = min(z_minimo,min(pts(:,3)));
